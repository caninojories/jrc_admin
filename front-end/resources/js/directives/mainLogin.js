...
angular.module('ngAPI')
  .directive('mainLogin', function ($routeParams, serviceRestAdmin, Restangular, loginService) {
    return {
      restrict : "E",
      replace : true,
      controller : function ($scope,$location, $route,  $timeout) {
        $scope.item;
          ...
          if($scope.loginForm.$valid) {
        		$scope.result = serviceRestAdmin.all('users').post({email:email, password: password}).then(function (valid) {
